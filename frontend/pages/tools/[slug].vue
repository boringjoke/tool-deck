...
const paperSizeInfo = {
  usage: [
    '使用搜索框输入纸张名称、英文名或别名，也可以按标准和系列筛选目录。',
    '在毫米和英寸之间切换，查看每个条目的短边 × 长边标准尺寸；表格和卡片视图可以按需要切换。',
    '点击“恢复全部”清除搜索和筛选，恢复毫米、表格视图和完整目录。',
  ],
  notes: [
    '首版包含 ISO 216 A0–A10、B0–B10，以及北美 Letter、Legal、Tabloid；不包含信封、照片纸、其他地区标准或自定义尺寸。',
    '尺寸按短边 × 长边展示，不代表横向/纵向排版；英寸值由毫米值换算并最多显示 2 位小数。',
    '工具只提供标准尺寸参考，不代表打印机可打印区域、纸张重量、裁切出血、打印兼容性或库存信息。',
    '数据来源、版本、复核日期和适用范围在页面中公开展示；查询只在当前浏览器内存中进行，不请求网络或写入浏览器存储。',
  ],
}

const worldCapitalsInfo = {
  usage: [
    '使用搜索框输入国家、首都、英文名或别名，也可以按地区筛选目录。',
    '结果按国家分组展示；一个国家存在多个首都时会在同一条记录中列出，并在必要时说明政府所在地、立法所在地等角色。',
    '点击“恢复全部”清除搜索、地区筛选和视图状态，恢复完整目录。',
  ],
  notes: [
    '首版覆盖 193 个联合国会员国和 2 个非会员永久观察员，共 195 个国家/实体记录；包含已核对的 200 条首都条目。',
    '地区只用于目录筛选，不代表政治归属或承认立场；没有法定首都或存在例外的记录以来源说明和中性角色标注。',
    '数据来源、版本、复核日期和适用范围在页面中公开展示；查询只在当前浏览器内存中进行，不请求网络或写入浏览器存储。',
  ],
}

const bloodTypeInheritanceInfo = {
  usage: [
    '分别选择家长 A 和家长 B 的常见血型，查看简化遗传模型下子代可能出现的血型集合。',
    '结果只展示可能性，不展示概率；交换两位家长的输入不会改变结果。',
    '需要重新选择时点击“恢复全部”，会清除两位家长的血型和当前结果。',
  ],
  notes: [
    '首版覆盖 A、B、AB、O 四种 ABO 表现型与 RhD 阳性/阴性组合，共 8 种常见血型；不提供未知血型、罕见亚型或其他血型系统。',
    '计算使用常见 ABO + RhD 的简化教学模型，不表示概率、医学诊断、输血配型、亲子鉴定或孕产风险判断。',
    'weak D、partial D、Bombay 表型和其他临床复杂情况不在首版范围；实际血型和相关医学问题请以正规检测及医务人员意见为准。',
    '计算、输入和结果只保留在当前浏览器内存中，不请求网络、不上传内容、不写入 URL 或浏览器存储。',
  ],
}

const periodicTableInfo = {
  usage: [
    '使用搜索框输入元素中文名、英文名、元素符号或原子序数，也可以按元素分类、区块和常温状态筛选。',
    '点击周期表中的元素，或用键盘聚焦后按 Enter/Space，在下方查看基础属性和标准原子量说明。',
    '点击“恢复全部”清除搜索、筛选和当前选中的元素；按 Esc 只清除当前选中。',
  ],
  notes: [
    '首版覆盖 118 个已命名元素，使用 18 列长表周期表；镧系和锕系在主表下方单独展示。',
    '详情包含原子序数、符号、中英文名、标准原子量、周期、族、区块、元素分类和常温状态；区间、约值和方括号质量数保留来源语义。',
    '104–118 的常温状态标记为未知/预测；不提供电子排布、密度、熔沸点、氧化态、同位素列表、化学计算或实验安全建议。',
    '目录、筛选和选中状态只保留在当前浏览器页面内存中，不请求网络、不上传内容、不写入 URL 或浏览器存储。',
  ],
}

const historicalDynastiesInfo = {
  usage: [
    '使用搜索框输入朝代或政权名称、别名，也可以按历史阶段和主线/并行轨道筛选。',
    '点击时间轴中的记录，或用键盘聚焦后按 Enter/Space，在下方查看年代、阶段、轨道和范围说明。',
    '点击“恢复全部”清除搜索、筛选和当前选中；按 Esc 只清除当前选中记录。',
  ],
  notes: [
    '首版覆盖夏、商、周至清的主要主线朝代，并以聚合或主要记录展示三国、十六国、南北朝、五代十国、辽、西夏、金、大理、北元和南明等并行关系。',
    '夏商周及其他存在口径差异的记录保留“约”“区间”或传统纪年说明，不提供虚假的单年精度。',
    '首版不提供逐帝目录、人物百科、疆域地图、战役年表、完整地方政权表或现代中国历史。',
    '目录、筛选和选中状态只保留在当前浏览器页面内存中，不请求网络、不上传内容、不写入 URL 或浏览器存储。',
  ],
}
            ...
            : tool.slug === 'uuid-generator'
              ? uuidInfo
                ...
                : tool.slug === 'json-to-sql'
                  ? jsonToSqlInfo
                : tool.slug === 'paper-size'
                  ? paperSizeInfo
                : tool.slug === 'world-capitals'
                  ? worldCapitalsInfo
                : tool.slug === 'blood-type-inheritance'
                  ? bloodTypeInheritanceInfo
                : tool.slug === 'periodic-table'
                  ? periodicTableInfo
                : tool.slug === 'historical-dynasties'
                  ? historicalDynastiesInfo
