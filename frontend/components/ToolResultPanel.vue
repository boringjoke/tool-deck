...
withDefaults(defineProps<{
  ...
  ariaLive?: 'polite' | 'off'
  showContentOnIdle?: boolean
  showContentOnProcessing?: boolean
  ...
  ariaLive: 'polite',
  showContentOnIdle: false,
  showContentOnProcessing: false,
  preserveContentOnError: false,
})
</script>

<template>
  <section class="tool-panel tool-result-panel" :aria-live="ariaLive">
    ...
    <div class="tool-panel__body">
      <div v-if="status === 'idle' && !showContentOnIdle" class="tool-state tool-state--empty">{{ emptyText }}</div>
      <div v-else-if="status === 'processing' && !showContentOnProcessing" class="tool-state">{{ processingText }}</div>
      <template v-else>
        <ErrorNotice v-if="status === 'error' && error" :error="error" />
        <template v-if="status !== 'error' || preserveContentOnError">
          <slot />
